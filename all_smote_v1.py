...
def generate_samples_zhou( X, nn_data, nn_num, rows, cols, steps,n_samples,
                new_n_maj,danger_and_safe):
    ...
    if danger_and_safe is not None:     #如果是boderline就去掉噪声点
        nn_data = _safe_indexing(nn_data, danger_and_safe)  # 边界点和安全点
    else:pass           #如果不是borderline就对所有少数类点进行过采样
    ...
    for nn in range(len(nn_data)):      #每个点的横纵坐标值
        ...
        nn_point = nn_data[nn]      #当前选择的母节点
        nn_weight = new_n_maj[nn]       #当前亩节点的权重
        # print(nn_point)
        # print(num)
        for i in range(n):            #随机有放回的抽取N次近邻点
            ...
            # random_float = round(random.uniform(0,1),len(str(n_samples)))   #取0-1之间的随机数
            ...
            if X_new_zhou  in X_new_1:
                X_new_zhou = np.array(nn_point) + (np.array(random_point_data) - np.array(nn_point))*proportion*round(random.uniform(0,1),len(str(n_samples)))*round(random.uniform(0,1),len(str(n_samples)))  
    ...
    for mm in range(m):
        ...
        nn_points = random.choice(nn_data)                       #有放回地抽取
        ...
        if (nn_point_weight+random_point_weights)!=0:
        ...
        X_new_zhou = np.array(nn_points) + (np.array(random_point_data)-np.array(nn_points)) * proportion*round(random.uniform(0,1),len(str(n_samples)))  
        if X_new_zhou  in X_new_1:
            X_new_zhou = np.array(nn_point) + (np.array(random_point_data) - np.array(nn_point))*proportion*round(random.uniform(0,1),len(str(n_samples)))*round(random.uniform(0,1),len(str(n_samples)))  
...
def generate_samples_zhou_borderline( X, nn_data, nn_num, rows, cols, steps,n_samples,
    ...
    '''
        ...
        #剩下的m个点用来随机不放回抽点，计算抽到的点和近邻点的权重比,每个点只插一次
    '''
    # print('nn_data:\t',len(nn_data))        #所有的少数类点
    # print('danger_and_safe:\t',np.flatnonzero(danger_and_safe))       #边界点在所有少数类点中的索引
    ...
    #随机不放回抽取m个点
    for mm in range(m):
        if isinstance(danger_data, list):pass
        else:danger_data = danger_data.tolist()
...
def generate_samples_zhou_svm_smote( X, nn_data, nn_num, rows, cols, steps,n_samples,
    ...
    for nn in range(lenth):      #每个点的横纵坐标值
        ...
        for i in range(n):            #随机有放回的抽取N次近邻点
            ...
            random_point_weight = new_n_maj[random_point]           #随机点权重
            random_point_data = nn_data[random_point]               #随机点的横纵坐标
            # print('随机节点:\t',random_point,random_point_data,random_point_weight)
...
class BorderlineSMOTE(BaseSMOTE):
    ...
    def _fit_resample(self, X, y):
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            ...
            noise_index = np.flatnonzero(noise== True)      #bool矩阵
            # print('noise_index:\t',noise_index)     #TODO
            danger_and_safe = (noise==False)                #bool矩阵
            # print('danger_and_safe:\n',danger_and_safe,type(danger_and_safe),len(danger_and_safe))
            ...
            # n_maj = conut_weight(n_maj=n_maj,noise_index=None)
            # print('所有点的权重:\t',n_maj)

            # print('边界点:\t',len(_safe_indexing(X_class, danger_index)))
            # print('权重矩阵:\t',len(new_n_maj))


            # 返回少数点在少数点中的K个近邻点的索引和距离           nn_k 是求少数类点中的k近邻点
...
SVMSMOTE # doctest: +NORMALIZE_WHITESPACE
    ...
    def _fit_resample(self, X, y):
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            if n_samples == 0:
                continue
            target_class_indices = np.flatnonzero(y == class_sample)#这个索引是在总体数据集X里面的索引
            ...
            support_vector = _safe_indexing(X, support_index)       #支持向量坐标
            # print('少数类中的支持向量：\t',len(support_vector))
            ...
            # print('边界点索引:\t',danger_index)
            # print('安全点索引:\t',safe_index)
            danger_list = []
            safe_list = []
            ...
            self.nn_k_.fit(X_class)     #用所有的少数类点来训练最近邻矩阵
            fractions = random_state.beta(10, 10)
            n_generated_samples = int(fractions * (n_samples + 1))
            # print('危险点数量：\t',np.count_nonzero(danger_bool))
            # print('安全点数量：\t',np.count_nonzero(safety_bool))
            ...
            # print('边界点的插值数量：\t',n_generated_samples,'\t安全点的插值数量:\t',n_samples - n_generated_samples,)
            # print('所有少数类点的权重矩阵:\t',new_n_maj,len(new_n_maj))
            ...
            if np.count_nonzero(danger_bool) > 0:
                ...
                X_new_1, y_new_1 = make_samples_zhou(
                    ...
                    y_dtype=y.dtype,
                    y_type=class_sample,
                    nn_data=X_class,                #所有近邻点的横总坐标,一般都是所有少数类
                    nn_num=nns,                     #近邻点的索引
...
class SMOTE(BaseSMOTE):
    ...
    def _fit_resample(self, X, y):
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():
            ...
            self.nn_m_.fit(X)       #在所有点中求少数点的近邻点，以此来求少数点的权重
            noise,n_maj = self._in_danger_noise(
                self.nn_m_, X_class, class_sample, y, kind="noise" 
            )
            ...
            # print('原始smote里面：\t',len(nns),len(nns),nns)
...
class KMeansSMOTE(BaseSMOTE):
    ...
    def _fit_resample(self, X, y):
        ...
        for class_sample, n_samples in self.sampling_strategy_.items():#也适用于多分类
            '''聚类'''
            # print('kemans需要插的样本类:\t',class_sample,'\n需要插的数量:\t',n_samples)

            if n_samples == 0:      #不需要插值就跳过，插下一类
                continue
            ...
            for valid_cluster_idx, valid_cluster in enumerate(valid_clusters):      #分簇和对应的标签
                ...
                # print('每个少数类点中多数类点的个数:\t',len(n_maj))
                ...
                def conut_weight(n_maj, noise_index):
                    # print(type(n_maj),type(noise_index))      #ndarray
                    # n_maj = np.delete(n_maj, noise_index)  #TODO 删除噪声点
                    # print(n_maj,len(n_maj))
                    new_n_maj = [round((1 - i / self.k_neighbors), 2) for i in n_maj]
